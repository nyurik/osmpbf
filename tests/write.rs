...
use crate::common::{assert_bytes, to_header, ExpBlob, ExpGroup, ExpNode, ExpRel, ExpWay};
use osmpbf::bbox::Bbox;
use osmpbf::write_block::PrimitiveBlock;
use osmpbf::{HeaderBlock, RelMemberType};
...
fn one_relation() {
    ...
    block.add_relation(
        ...
        vec![
            ("abc".to_string(), "def".to_string()),
            ("def".to_string(), "abc".to_string()),
        ],
        vec![("".to_string(), 1, RelMemberType::RELATION)],
    );
    assert_bytes(
        block.finalize().as_slice(),
        &[ExpBlob::Data(vec![{
            ExpGroup {
                ...
                rels: vec![ExpRel {
                    id: 42,
                    members: vec![(1, RelMemberType::RELATION, "".to_string())],
                    tags: vec![
                        ("abc".to_string(), "def".to_string()),
                        ("def".to_string(), "abc".to_string()),
                    ],
